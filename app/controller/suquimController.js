app.controller('suquimController', function ($scope, SuquimFactory, DateService, suppliersFactory, NotificationService) {


    // bind with model
    $scope.equipments = SuquimFactory.equipments;
    $scope.recentEquipments = SuquimFactory.recentEquipments;

    $scope.suppliers = suppliersFactory.suppliers;

    // Tabs selection
    $scope.tabSelected = SuquimFactory.tabSelected;
    $scope.selectTab = tab => {
        if (tab != SuquimFactory.tabSelected) {
            SuquimFactory.selectTab(tab);
            $scope.tabSelected = SuquimFactory.tabSelected;
            $scope.items = null;
    ...
    $scope.search = () => {
        $scope.loading = true;
        SuquimFactory.search([$scope.searchArray, $scope.ppmValue]).then(() => {
            $scope.loading = false;
        })
    }
    ...
    $scope.submit = () => {
        switch ($scope.modalTitle) {
            case 'Add Equipment':
                SuquimFactory.submitAddEquipment($scope.modalData).then(response => {
                    if (response) {
            ...
            case 'Edit Equipment':
                SuquimFactory.submitEditEquipment($scope.modalData);
                equipmentsModal.hide();
                SuquimFactory.search([$scope.searchArray, $scope.ppmValue]);
                SuquimFactory.fetchRecent();
                break;
        }
    }

    $scope.deleteEquipment = () => {
        SuquimFactory.deleteEquipment($scope.modalData.record_ID).then((response) => {
            if (response == 1) {
                equipmentsModal.hide();
                SuquimFactory.search([$scope.searchArray, $scope.ppmValue]);
            }
    ...
    $scope.moreInfoModal = data => {
        ...
        SuquimFactory.getExtensions(data.record_ID).then(response => {
    ...
    // add extension
    $scope.addExtension = () => {
        SuquimFactory.addExtension($scope.newExtensionData).then(response => {
            if(response == 'added') {
                infoModal.hide()
            }
        })
    ...
    $scope.deleteExtension = (index, ID) => {
        NotificationService.showWarning().then(res => {
            if (res.isConfirmed) {
                SuquimFactory.deleteExtension(ID).then(res => {
                    if (res == 'deleted') {
    ...
    $scope.openServiceModal = data => {
        ...
        SuquimFactory.getService(data.record_ID).then(response => {
    ...
    $scope.submitNewService = () => {
        SuquimFactory.submitNewService($scope.newServiceData).then(response => {
            if (response == 'added') {
                serviceModal.hide();
            }
        })
    ...
    $scope.deleteService = (index, ID) => {
        NotificationService.showWarning().then(res => {
            if (res.isConfirmed) {
                SuquimFactory.deleteService(ID).then(response => {
                    if (response) {
    ...
    // export excel
    $scope.exportExcel = () => {
        SuquimFactory.exportExcel($scope.equipments);
    }
